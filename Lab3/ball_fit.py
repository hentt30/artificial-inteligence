...
def fit_hill_climbing():
    ...
    def neighbors(theta):
        ...
        neighbors_list = []
        # Todo: Implement
        angulo=2*pi/num_neighbors
        seno=sin(angulo)
        cosseno=cos(angulo)
        vet= [0  ,0,  delta*seno,-delta*seno,delta*cosseno,-delta*cosseno,-delta,delta]
        vet2=[delta,-delta,delta*cosseno,-delta*cosseno,-delta*seno,delta*seno,  0 ,    0 ]
        for i in range(num_neighbors):
           v=theta[0]+vet[i]
           z=theta[1]+vet2[i]
           vector=np.array([v,z])
            
           neighbors_list.insert(i,vector)
...
def fit_simulated_annealing():
    ...
    def random_neighbor(theta):
        ...
        :rtype: numpy.array.
        """
        angulo=random.uniform(-pi,pi)
        v=theta[0]+delta*cos(angulo)
        z=theta[1]+delta*sin(angulo)
        aux=[v,z]
        return aux
