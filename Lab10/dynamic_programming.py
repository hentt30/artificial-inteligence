    ...
    while n < num_iterations:
        for i in range(dimensions[0]):
            for j in range(dimensions[1]):
                ...
                for action in range(NUM_ACTIONS):
                    value[current_state] += policy[current_state][action]*grid_world.reward(current_state,action)
                    for next_state in grid_world.get_valid_sucessors(current_state,action):
                            value[current_state] += grid_world.gamma*policy[current_state][action]*grid_world.transition_probability(current_state,action,next_state)*initial_value[next_state]
        if np.amax(np.absolute(value - initial_value)) < epsilon:
            break
        else:
    ...
    for n in range(num_iterations):
        for i in range(dimensions[0]):
            for j in range(dimensions[1]):
                ...
                for action in range(NUM_ACTIONS):
                    aux_value = 0
                    aux_value += grid_world.reward(current_state,action)
                    for next_state in grid_world.get_valid_sucessors(current_state,action):
                            aux_value += grid_world.gamma*grid_world.transition_probability(current_state,action,next_state)*initial_value[next_state]
